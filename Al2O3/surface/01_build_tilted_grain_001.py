...
class myclass:
	...
	def define_new_cell(self):
		print "natom = %d" % (self.at.atn)
		
		self.new_a = self.at.lt[2]
		self.new_b = 0.5*math.sqrt(self.at.lt[1]*self.at.lt[1] + self.at.lt[2]*self.at.lt[2])
		self.new_c = self.at.lt[0]
		self.new_ga = math.degrees(math.atan2(self.at.lt[1], self.at.lt[2]))

		self.at.lt[0], self.at.lt[2] = self.at.lt[2], self.at.lt[0]
		self.at.lv = atm.ltlv(self.at.lt, "vasp")
		self.at.x, self.at.z = list(self.at.z), list(self.at.x)
		self.at = atm.supercell(self.at, [2,1,1], "vasp")

		self.at = atm.frac_to_cart(self.at, "vasp")

		self.at.lt[0] = self.new_a
		self.at.lt[1] = self.new_b
		self.at.lt[2] = self.new_c
		self.at.lt[5] = self.new_ga
		self.at.lv = atm.ltlv(self.at.lt, "vasp")
		...
		e, x, y, z = [], [], [], []
		for i in range(self.at.atn):
			if -1e-4 <= self.at.x[i] <= 0.9999:
				if -1e-4 <= self.at.y[i] <= 0.9999:
					if -1e-4 <= self.at.z[i] <= 0.9999:
						e.append(self.at.e[i])
						x.append(self.at.x[i])
						y.append(self.at.y[i])
						z.append(self.at.z[i])
